...
struct CharacterScore {
	GLuint TextureIDScore; // ID handle of the glyph texture
	ivec2 SizeScore; // Size of glyph
	ivec2 BearingScore; // Offset from baseline to left/top of glyph
	GLuint AdvanceScore; // Offset to advance to next glyph
};
...
private:
	...
	GLuint VBO, VAO, EBO, texture, program, VBO2, VAO2, EBO2, texture2, program2, VBO3, VAO3, EBO3, texture3, program3, VBO4, VAO4, EBO4, texture4, program4
		, VBO5, VAO5, EBO5, texture5, program5, VBO6, VAO6, EBO6, texture6, program6, VBOscore, VBO2score, VAOscore, VAO2score, programscore;
	...
	map<GLchar, CharacterScore> CharactersScore;
	void InitTextScore();
	void RenderTextScore(string text, GLfloat x, GLfloat y, GLfloat scale, vec3 color);
